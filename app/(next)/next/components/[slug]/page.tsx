...
export default async function ComponentPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <div className="prism-content"><div className="prism-page-heading"><h1>{item.title}</h1><p>{item.summary}</p><p className="!text-sm">{item.kind==='extension'?'Origin Tree 适配扩展':item.kind==='pattern'?'coss 组合示例':'coss 原始组件'} · 已实现，待评审</p></div><DemoRenderer id={slug}/><footer className="prism-source mt-10 border-t pt-5">实现基于 <a href={item.sourceUrl??('https://coss.com/ui/docs/components/'+slug)} target="_blank" rel="noreferrer">{item.kind==='extension'?'coss Origin Tree 示例':'coss '+slug+' 官方文档'} ↗</a>。使用页面顶部选择器检查浅色、暖纸与深色。</footer></div>
}
