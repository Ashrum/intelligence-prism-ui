...
const componentRoles = [
  ...
  ["Input / Field", "Input 接收值，Field 组织字段身份、约束与反馈，场景决定校验与应用时机。默认边界标识可填写区域；只读与禁用字段沿用所在表面，以轻实线边界和只读／不可用文字说明模式。只读内容保持正常文字，禁用文字适度减弱。采用已确认的 M3 Outlined 浮动标签与舒适／紧凑密度，边框、标签、光标和错误图标联动。接受就近修正错误，继续编辑时保留上次结果并标明未应用修改；拒绝只靠 Placeholder 标识字段、只用 Toast 说明字段错误，或把开始编辑当成已应用。"],
