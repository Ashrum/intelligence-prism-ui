...
const behaviors = [
  ["识别入口", "在默认状态下就能分清阅读内容、链接和可填写区域。", "接受链接常显下划线、输入常显边界和标签、原位编辑保留明确入口；拒绝必须先 Hover、点击或聚焦，才能发现可以操作。"],
...
const componentRoles = [
  ["链接与文本操作", "正文和独立文字链接默认使用操作色与常显下划线；文本操作仍保留按钮语义。导航依靠稳定位置、结构与当前项说明用途。接受不先进行操作就能找到入口；拒绝让可点击文字只在 Hover 时与正文不同。"],
  ...
  ["Input / Field", "Input 接收值，Field 组织字段身份、约束与反馈，场景决定校验与应用时机。默认边界标识可填写区域；只读显示中性底、间断边框与只读文字，禁用显示不可用文字和禁用表面。采用已确认的 M3 Outlined 浮动标签与舒适／紧凑密度，边框、标签、光标和错误图标联动。接受就近修正错误，继续编辑时保留上次结果并标明未应用修改；拒绝只靠 Placeholder 标识字段、只用 Toast 说明字段错误，或把开始编辑当成已应用。"],
...
export function DesignLanguageRecord() {
  return <article id="design-language" className="prism-language-record" aria-labelledby="language-title">
    ...
    <section className="language-record-section" aria-labelledby="language-character-title">
      ...
      <p>可操作性在操作之前就要可识别。链接默认有操作色与下划线，输入区在空值、已有值和失焦时均保留可见边界；只读与禁用各自明确说明。Hover 与 Focus 增强已有线索，不承担首次发现入口的职责。原位编辑保持阅读字体与行宽，同时常显编辑入口，并在进入编辑后清楚显示输入边界。</p>
      <p>用来识别可输入区域的边界，与相邻背景至少达到 3:1 对比；普通内容容器可以继续采用较轻边界。当前浅色主题通过 <code>--field-border</code> 区分输入边界与容器边界。参考 <a href="https://www.w3.org/WAI/WCAG22/Understanding/non-text-contrast.html">W3C 非文本对比说明</a>；数值检查不能代替真实页面的可识别性评审。</p>
