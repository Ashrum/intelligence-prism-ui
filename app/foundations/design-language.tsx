...
const componentRoles = [
  ...
  ["Card、列表、表格", "帮助识别对象、比较差异和作出判断。Card 按任务取舍区块，指标、资源或静态概览不强制套用复核场景的状态与操作结构。Card 中的身份与范围在选择、展开、编辑期间保持可见，状态变化只影响当前对象；AI 来源不要求整卡上色。接受同类指标沿共同列对齐、失败保留已有结果、修改与复核分别表达；拒绝每项再套一张卡导致无法横向比较，或用一次操作改动无关对象的状态。"],
...
export function DesignLanguageRecord() {
  return <article id="design-language" className="prism-language-record" aria-labelledby="language-title">
    ...
    <section className="language-record-section" aria-labelledby="language-character-title">
      ...
      <p>阅读与可操作文字以 7:1 为对比目标，辅助信息、占位文字和操作文案同样保持清晰；字号、字重、间距与位置共同组织层级。输入、选择和焦点的必要识别边界，与相邻背景至少达到 3:1 对比；普通内容容器可以继续采用较轻边界。禁用态使用浅底与轻边界，文字保持 4.5:1 以上，并保留不可用说明。当前浅色主题通过 <code>--field-border</code> 区分输入边界与容器边界。参考 <a href="https://www.w3.org/WAI/WCAG22/Understanding/contrast-enhanced.html">W3C 文字增强对比</a>与<a href="https://www.w3.org/WAI/WCAG22/Understanding/non-text-contrast.html">非文本对比说明</a>；按真实背景与透明度合成后的颜色检查，数值检查不能代替真实页面的可识别性评审。</p>
