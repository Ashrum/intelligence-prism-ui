...
export type ReviewBasketAdapter = { open: boolean; position: 'right'|'bottom'; sampleAction: ReactNode }
export function WorkbenchReview({ returnHref='/next/skeletons', returnLabel='页面骨架', basket: externalBasket }: { returnHref?: string; returnLabel?: string; basket?: ReviewBasketAdapter }) {
 ...
 const basket = externalBasket ?? { open:basketOpen, position:wide?'right' as const:'bottom' as const }
 ...
 <WorkbenchShell organization={{ name:longName?'启明实验学校教育集团高中部数学教研中心（东湖校区）':'启明实验学校', description:'教师工作台 · 组织示例', mark:<School className="size-6" /> }} user={{name:'王建国',role:'数学教师',initials:'王'}} navigation={navigation} activeId={active} onNavigate={choose} onPersonal={()=>setAnnouncement('个人页面入口已触发。第7项尚未启动，本轮不提供个人页面正文。')}
  ...
  auxiliary={<div className="space-y-7"><div><h2 className="text-sm font-semibold">评审条件</h2><p className="mt-2 text-sm text-muted-foreground leading-relaxed">切换样例，检查公共区域的不同状态。</p></div>
    ...
    <p className="text-xs text-muted-foreground leading-relaxed">第1项 · 总骨架 v0.1 候选<br />第2—7项未启动</p>
  </div>}>
   <div className="mb-9 flex flex-wrap items-center justify-between gap-3"><Button variant="ghost" size="sm" render={<a href={returnHref} />}><ArrowLeft />{returnLabel}</Button><Badge variant="outline">交互演示</Badge></div>
   <div className="mb-10"><p className="text-sm text-muted-foreground">教师工作台 / 总骨架</p><h1 className="mt-3 text-3xl font-semibold tracking-tight">教师工作台总骨架</h1><p className="mt-4 max-w-xl text-base text-muted-foreground leading-relaxed">导航、公共工具与内容区域的组合评审。正文使用少量阅读样例，验证日常教学中的文字、公式与信息密度。</p></div>
   <p role="status" className="mb-7 text-sm text-info-foreground leading-relaxed">{announcement || '搜索、通知与状态详情均可操作；所有内容与状态均为演示。'}</p>
   <section ref={sample} aria-labelledby="review-sample-title" className="space-y-6">
    <div><p className="mb-3 text-xs text-muted-foreground">阅读样例 01 / 高中数学</p><h2 id="review-sample-title" className="text-xl font-semibold leading-relaxed">函数与不等式：从条件整理到结论核对的课堂材料</h2><p className="mt-3 text-sm text-muted-foreground">示例材料 · 2026.09.19 · 仅用于总骨架评审</p></div>
    <p className="text-base leading-8">在整理课堂材料时，先明确变量的取值范围，再检查每一次变形是否满足条件。较长的中文说明应该自然换行；公式需要与上下文保持清楚的阅读关系。</p>
    <div className="review-equation py-6" aria-label="已知函数 f(x) 等于 x 的平方减 2x 加 1，且 x 为实数，则 f(x) 大于等于 0。"><math display="block"><mrow><mi>f</mi><mo>(</mo><mi>x</mi><mo>)</mo><mo>=</mo><msup><mi>x</mi><mn>2</mn></msup><mo>−</mo><mn>2</mn><mi>x</mi><mo>+</mo><mn>1</mn><mo>=</mo><msup><mrow><mo>(</mo><mi>x</mi><mo>−</mo><mn>1</mn><mo>)</mo></mrow><mn>2</mn></msup><mo>≥</mo><mn>0</mn></mrow></math></div>
 ...
 {!externalBasket && <>
  ...
  <QuestionWorkPanel id="shell-review-basket" open={basketOpen} position={wide?'right':'bottom'} title="试题篮" description="布局演示 · 与工作台使用同一工作面板" initialFocus={false} finalFocus={()=>launcher.current ?? false} showCloseButton={false} onClose={()=>setBasketOpen(false)} className={`review-basket-panel review-basket-${wide?'right':'bottom'}`} footerVariant="bare" footer={<div className="h-10" />}>
   {selected ? <div className="space-y-4"><div className="flex items-center justify-between gap-3"><Badge variant="outline">示例题</Badge><Button size="icon" variant="ghost" aria-label="移出示例题" onClick={()=>{setSelected(false);launcher.current?.focus()}}><X /></Button></div><p className="text-base leading-8">已知 <math><mi>f</mi><mo>(</mo><mi>x</mi><mo>)</mo><mo>=</mo><msup><mi>x</mi><mn>2</mn></msup><mo>−</mo><mn>2</mn><mi>x</mi><mo>+</mo><mn>1</mn></math>，求函数的最小值。</p><p className="text-sm text-muted-foreground">仅验证选题与空间，本页不进入编辑发布流程。</p></div> : <p className="py-6 text-sm text-muted-foreground leading-relaxed">还没有选题。可在正文中加入示例题，页面保持可操作。</p>}
  </QuestionWorkPanel>
 </>}
 </>
}
