...
export function AgentReview({returnHref='/next/skeletons',returnLabel='页面骨架',basket:externalBasket,onNavigate}: {returnHref?:string;returnLabel?:string;basket?:ReviewBasketAdapter;onNavigate?:(id:string)=>void}) {
 ...
 const messageContent=<div className="space-y-8" aria-label="当前对话消息">{current.messages.map(m=><article key={m.id} aria-label={m.role==='user'?'我的消息':'Agent 回复'} className={m.role==='user'?'ml-auto max-w-[90%] space-y-2':'space-y-3'}>
  <p className="text-xs text-muted-foreground">{m.role==='user'?'我':'Agent · 固定示例回复'}</p>
  {m.state==='replying'?<div role="status" className="flex items-center gap-2 text-sm text-muted-foreground"><Spinner/>正在演示回复…</div>:<p className="text-base leading-8 whitespace-pre-wrap break-words">{m.text}</p>}
  {m.material&&m.role==='user'&&<Badge variant="outline"><FileText/>二次函数课堂材料 · 示例</Badge>}
  {m.role==='assistant'&&m.state==='ready'&&<>
   {m.material&&<div className="space-y-4"><div className="overflow-x-auto py-2" aria-label="f(x) 等于 (x 减 1) 的平方，大于等于 0"><math display="block"><mi>f</mi><mo>(</mo><mi>x</mi><mo>)</mo><mo>=</mo><msup><mrow><mo>(</mo><mi>x</mi><mo>−</mo><mn>1</mn><mo>)</mo></mrow><mn>2</mn></msup><mo>≥</mo><mn>0</mn></math></div><p className="text-sm leading-7">当 <math><mi>x</mi><mo>=</mo><mn>1</mn></math> 时取等号。此处为已有材料的固定样例，没有生成或发布业务成果。</p>{sampleAction}</div>}
...
function ConversationDirectory({shown,active,historyQuery,setHistoryQuery,onNew,onSelect,close}:{shown:DemoConversation[];active:string;historyQuery:string;setHistoryQuery:(v:string)=>void;onNew:()=>void;onSelect:(id:string)=>void;close:()=>void}) {
 ...
 return <div className="space-y-5">
  ...
  <nav aria-label="对话列表" className="space-y-2"><p className="px-3 text-xs text-muted-foreground">最近对话 · 本页示例</p>{shown.map(c=><Button key={c.id} variant={c.id===active?'secondary':'ghost'} className="h-auto sm:h-auto w-full justify-start py-2 text-left whitespace-normal" aria-current={c.id===active?'page':undefined} onClick={()=>{onSelect(c.id);close()}}><span className="min-w-0 break-words">{c.title}</span>{c.messages.some(m=>m.state==='replying')&&<Spinner className="shrink-0"/>}</Button>)}{!shown.length&&<p role="status" className="px-3 text-sm text-muted-foreground">没有匹配的对话。</p>}</nav>
  <p className="px-3 text-xs text-muted-foreground leading-relaxed">仅本次页面会话，刷新后恢复示例。</p>
 </div>
}
