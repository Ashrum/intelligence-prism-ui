...
export function getReviewStatus(editor: ReviewEditor, limits: Record<string, number>) {
  ...
  const state = missing.length ? "incomplete" : invalid.length ? "invalid" : changed.length ? "changed" : editor.reason.trim() ? "note" : editor.record ? "confirmed" : "pending"
