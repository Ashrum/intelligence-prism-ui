...
## v1.21.0 — 三元素状态与全目录质量提升

- 状态复用 coss Badge：紫红待处理、曜蓝进行中、青绿已完成；草稿/暂停为中性，风险琥珀与错误红保留独立语义。文字与图标同时表达，状态不只靠颜色。
- 接入 Card、表格、题目复核、诊断、目标、计划、Agent、里程碑、材料保存与全部组件页评审状态；基础规范同步说明。
- 品牌 ink/surface 提升为共享 token，保留题型旧 token 别名；深色进度/度量、反馈图标和报错文字使用可读前景。
- 全站示例说明降为辅助层级；指标按容器自动排列，里程碑宽屏横排/窄屏竖排。
- 数据表支持明确的数值列对齐与可读记录名称；组成图选中图例增加勾选和背景；按钮示例消除空操作和虚假反馈。
- 保留 80 个目录项、三主题及既有页面，未新增依赖，coss 原始组件保持不变。

## v1.20.3 — Card 视觉复核修正

- 示例说明降为辅助层级，材料标题与正文保持主要阅读权重；六种示例按用途配对。
- 网格按实际容器宽度切换单列／双列；配对展示时选择项纵向排列，窄屏卡片保留流动宽度。
- 三元素恢复既定的 21×30px 标记与原笔画比例，文字头像增加细轮廓并保留无底色。
- 进度使用三主题配对蓝色，提高完成区与剩余区对比；128/160 与 80% 数值保持一致。
- 同步已有 GitHub 文档中的 beUI 外部交互参考，未增加运行依赖。

## v1.20.2 — Card 构图与阅读节奏

- 内容卡三元素签名移到角部，标题、分类与正文共用左边界；状态与时长成组，收藏在左、阅读在右。
- 指标复用 Progress，128/160 推导为 80%，保留中性未完成区；人物简介独立于身份行。
- 紧凑资源卡与可选卡收紧间距，正文仍为 16px；宽屏指标与人物示例并排，卡片随容器伸缩。
- 分组入口复用固定比例三元素，状态与品牌语义保持独立；没有新增页面、依赖或 coss 变体。

## v1.20.1 — Card 示例整合

- 内容卡状态进入题头，分类、时长与更新状态分开；材料主操作统一为蓝色。
- 指标卡去掉外框和无实际刷新行为的“持续更新”徽章；人物卡采用统一阅读轴。
- 分组标题独立于内层卡片，可选卡的蓝色选中边界与键盘焦点分别表达。
- 三元素签名复用于品牌内容卡，人物身份使用紫红配色；状态继续遵循既有语义。
- 原位更新六种示例，复用 coss Card、CardFrame、Avatar、RadioGroup 与 Badge。

## v1.20.0 — 全站视觉与交互统一

- 三元素以固定颜色与笔画比例复用到品牌、总览与基础规范；总览加入真实组件预览。
- 主操作统一蓝底白字；品牌紫红与错误红分离，三主题各使用可读的文字配色。
- 减少示例重复外框，压缩页面元信息；导航按组折叠，支持原位组件搜索与键盘打开。
- 热力图采用连续色阶和自适应文字颜色，缺测以斜纹区分；图表选择反馈显示名称。
- 试题篮明确区分篮内总量与勾选量，组卷按钮显示实际使用的题数。

## v1.19.0 — 复核信息层级与状态

- 复用现有 QuestionReview，区分题面、得分、参考答案与理由输入；宽窄容器自适应。
- 逐评分项识别待确认修改，区分待复核、缺分、未提交说明和已确认；历史差异不持续报警。
- 评分上限与输入相邻，初评和已记录差值清晰展示；错误关联至对应评分项或理由字段。

## v1.18.1 — 精简题篮与修复 HTML 导出

- 移除篮内详情及其跨区域定位、临时预览和返回逻辑，题篮保留核对、勾选与移出操作。
- 去掉题间分割线，以序号、标题与留白区分题目；保留统计区和底部操作区的边界。
- HTML 导出明确使用自动 JSX 转换，修复未定义 React 引起的空白；初始文档包含 12 张题卡和数学公式，交互由同一组件接管。

## v1.18.0 — 解析对齐与试题篮布局

- 答案与解析中的独立公式与正文左边缘对齐，行内公式及数学结构保持原样；独立题卡和打印答案共用此规则。
- 试题篮复用 `QuestionCard` 与已有操作：序号独立，标题、题型分值和题干对齐，以题间细线替代逐张卡片外框。保留勾选、移出、详情回返和草稿隔离。
- 常规高度下，顶部统计与组卷操作、底部辅助操作固定，题目列表独立滚动；短窗口改为整篮滚动，保留收起后的位置。

## v1.17.1 — 移除题卡内分割线

- 移除题面与底部操作之间的分割线，使用原有留白分隔；独立题卡、列表与试题篮同步生效。

## v1.17.0 — 参考题卡基准

- 题型与分值在上、标题在下；采用参考的 8px 细边框、22/24/18px 内边距、16px 标题、15px 正文和 14px 选项与小问。
- 题卡与列表取消 48rem 限宽，选项随容器换列，宽容器中的几何材料可并排；所有场景共用同一题卡。
- 保留曜蓝配色、真实勾选/收藏/作答逻辑与数学渲染，更新原有独立 HTML。

## v1.16.1 — 题面阅读排版（宽度方案已由 v1.17.0 撤回）

- 题卡、列表分隔线和操作区统一限制在 48rem 内，题干正文约 716px；窄面板继续自适应。
- 正文保留 16px，收紧行距与段距，调整三主题正文灰度；小问选项复用父题的对齐和容器换列规则。
- 打印版保留独立正文颜色和段落规则，独立 HTML 同步更新。

## v1.16.0 — 更多题目与独立 HTML

- 题目组件示例扩充为 12 道，新增集合、概率、数列、三角函数、指数方程和统计题；在同一列表展示，支持按题型筛选。
- 复用同一组件导出单文件 HTML，内嵌样式、脚本和数学字体，可离线打开并切换三主题。生产构建后运行 `node scripts/export-question-component.mjs /绝对路径/question-component.html`。

## v1.15.1 — 题卡样式统一

- 以原列表题卡为统一基准，独立展示、题库、试题篮和组卷使用相同标题、留白及操作排版；移除独立卡片外框与外观分支。
- 完整题面与摘要仅控制内容范围，摘要自然换行；组件页合并重复题卡示例。

## v1.15.0 — 题型色彩与操作层级

- 单选、多选、判断、填空使用曜蓝，解答使用曜紫红，已知混合小问使用曜青绿。分类依据是作答模型，业务题型名称原样显示；缺失或未知模型回退中性，带小问不自动归入复合题。
- 整题、列表、小问和试题篮共享题型标识；分值徽标针对浅色、暖纸、深色分别配色。主操作蓝底白字，详情与收藏保持低权重，相似题收进更多菜单。
- 修复多选小问误显示为解答，以及隐藏分值连同题型一起隐藏的问题。小问可继承明确的父级作答模型；零分和局部分值覆盖保留。
- 数学图仅点缀研究区域与关键动点，保留坐标关系。纸面使用纯文字类型标签与黑白图形；教师答案继续保留原题评分依据。没有新增页面、字体或依赖。

## v1.14.0 — 题目卡片与试题篮

- QuestionCard 引入圆形序号、独立标题、题型与分值标签、题干摘要和 `headerActions` 右侧操作插槽，并用于试题篮。
- 试题篮复用同一题卡，增加题数、原题总分和题型分布；固定底部保留清空、用于组练习、用于组卷。移除、详情回返、勾选和草稿隔离沿用原逻辑。
- 组件页同时提供独立卡片和可移除列表示例，继续使用原三主题与 coss 控件，未新增页面或依赖。

## v1.13.1 — 可用基线

- 80 个组件经用户确认评审通过，统一移除待评审状态。
