...
本分支按方案 A 保留题卡信息层级、选题篮交互、12 道题目 fixtures、组件分类与独立 HTML 导出。视觉沿用 origin/main 的三主题、语义令牌与组件公开变体；目录仍为 80 项（54 基础、16 组合、10 扩展）。本轮待 Supervisor 独立评审。

- 题型使用 outline Badge，分值使用 secondary Badge；状态组合使用已有 Badge 变体与图标，不新增视觉令牌。
- 复核按钮只发出确认意图；完成状态由外部 status 与记录提供。组件总览按钮明确标注示例操作，不表示保存。
...
分支原 v1.14–v1.21 的视觉候选已撤回；以下只登记本次保留的结构与行为，不作为视觉批准或发布记录。

- 题型与分值在上、标题在下；headerActions 提供题头操作。完整题面与 compact 摘要自然换行，选项按容器换列，宽容器材料可并排。
- 试题篮复用 QuestionCard，保留序号、题数与分值统计、勾选、移出、组练习和组卷；移除篮内详情及跨区域临时预览。常规高度列表滚动，短窗口整篮滚动并保留位置。
- 作答模型与业务题型名称独立；小问继承明确父级类型，缺失/未知不补造类型。隐藏分值仍保留题型，零分、局部分值覆盖与教师打印评分依据保留。
- QuestionReview 分别呈现初评、已记录和复核中得分；逐评分点比较，缺分、非法值、改分理由校验和取消草稿保留。完成状态只来自外部 status；示例记录与真实回执分开。
- 组件页提供 12 道自编题，支持题型筛选和选择／编排／作答／复核四种组合；没有 OCR、模型或自动判分。
- 生产构建后运行 `node scripts/export-question-component.mjs /绝对路径/question-component.html`，复用同一组件导出含样式、脚本及数学字体的独立 HTML；初始文档包含题卡和公式，交互使用自动 JSX 转换。
- Card 保留六类组合与容器自适应网格；目录保留预览、分组折叠和键盘搜索；里程碑保留容器响应式、可见状态与 aria-current。
- 数据表保留 numeric / align / rowLabel；图表保留 main 默认配色、明确零值与缺测、原暖纸候选接口和选择名称反馈。
- beUI 仅作外部交互参考，不引入依赖、组件或视觉规范。
