...
## 当前版本：coss v1.2.1（弹出式范围选择，设计待评审）
...
- 54 项 coss 官方基础组件、1 项 Date Picker 日期组合和 1 项教材目录扩展均有可操作示例。旧目录的 59 项规划是历史记录，不等同于本版交付范围。
...
- `node --test tests/*.test.mjs`：旧功能回归、全部 56 条组件、组合与扩展路由、新旧根隔离、原始 coss 散列、旧内容保留、材料校验和 Agent 状态约束。
