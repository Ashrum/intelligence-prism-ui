...
- 第1项待用户站点评审；第2—7项未启动。本次候选通过 PR 审核，不替换正式站点。

验证：两个项目类型检查和构建通过；新增语义模型 2 项测试、工作台 9 项 ID/题篮/导航回归通过。浏览器检查 1363px 桌面，以及实际页面在 1280/820/390px iframe 视口下的响应式表现、三主题、长组织名、长中文/公式、键盘搜索、通知已读与空态、任务五状态、积分/token 三状态、题篮展开和跨路由保留。修复搜索结果的焦点落点、HashRouter 跳过链接和 Flex 宿主宽度。实体移动设备、屏幕阅读器、真实后端与多用户服务未验证；不把构建通过视为用户视觉验收。
