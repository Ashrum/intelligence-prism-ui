...
| 12 | 选择与组合 | 结构编排器 | AgentStructureArranger（P/agent-structure-arranger.tsx）；Card / 列表 / Checkbox / NumberField / Select / Button / RecordDetails | Inline + 专用扩展内容 | inline / workspace / compact | 组件候选 | 源码已合入本地 main `a104c0d`（PR #79）；本轮未复核 Workspace 验收；原候选测试及报告 `.sites-runtime/structure-arranger/REPORT.md` | PO 2026-09-26 批准：下一批首项；矩阵 B05/D05/Q04 | 待 Supervisor 独立 Review、PO 确认与 Workspace D05/Q04 轻量接入；只读核对当前 Workspace main `0f7052a`，复用 `paper.update`／`paper.save`，分组持久化尚无模型支持。本轮不改 Workspace，不启动服务；三主题／窄屏实际交互、真实服务未验证。12 编排已有条目，35 编辑内容层级，11 管理集合成员。 返工停止项：写死“条目 N”前缀、共用来源逐项前缀、首尾移动含糊原因，已处理（本分支）；新增 scopeLabel 与 moveBoundaryReasons，Workspace 待同步并用“第 N 题”复验，未升级接入状态。 |
