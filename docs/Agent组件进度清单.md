...
支持视图列使用 `inline / workspace / compact` 表示实际用法。其中 01/02/03/04/06/07/08/10/11/12/14/16/17/18/19/21/22/26/27/28/33/35/36 的 API 是 `view="inline" / "workspace"` 加独立 `density="default" / "compact"`；compact 为紧凑密度，可与两种 view 组合，不是第三种业务态。`presentation="card" / "inline"` 只控制外框，不等于扩展态。

## 核查基线与证据

- 2026-09-26 本轮推进语义 12 为组件候选：`AgentStructureArranger`，分支 `feat/agent-structure-arranger`，基于 main `fd454db`。组→项两层、简单移动／拖拽增强、分组、数值／选项编排属性与批量请求；合计、校验、版本与保存事实由页面提供。采用第二轮说明合并、标题一次、原因一次、未知合并规则。五项日志及 Workspace 只读承载方案见 `.sites-runtime/structure-arranger/REPORT.md`；不把自动测试升级为 Workspace 或浏览器验收。
...
| 12 | 选择与组合 | 结构编排器 | AgentStructureArranger（P/agent-structure-arranger.tsx）；Card / 列表 / Checkbox / NumberField / Select / Button / RecordDetails | Inline + 专用扩展内容 | inline / workspace / compact | 组件候选 | 未合并任务分支 `feat/agent-structure-arranger`，基于 main `fd454db`；测试及报告 `.sites-runtime/structure-arranger/REPORT.md` | PO 2026-09-26 批准：下一批首项；矩阵 B05/D05/Q04 | 待 Supervisor 独立 Review、PO 确认与 Workspace D05/Q04 轻量接入；只读核对当前 Workspace main `0f7052a`，复用 `paper.update`／`paper.save`，分组持久化尚无模型支持。本轮不改 Workspace，不启动服务；三主题／窄屏实际交互、真实服务未验证。12 编排已有条目，35 编辑内容层级，11 管理集合成员。 |
