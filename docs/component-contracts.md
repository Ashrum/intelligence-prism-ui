...
- 独立展示、题库、试题篮和组卷共用同一无外框排版：标题字号、题型分值行、留白和操作插槽保持一致。题卡及列表最大宽度为 48rem，题面与操作共用阅读区域；正文保持 16px，屏幕题卡使用 1.8 倍行高与配对灰度，公式保留前景色。打印正文颜色和段距独立。列表分隔线由调用方容器提供；不再提供 `variant`、`reading` 外观分支。`number` 控制圆形序号；未提供序号和选择回调时不预留左栏。
