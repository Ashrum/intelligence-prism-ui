...
import { ArrowUpRight,BookOpen,MousePointerClick } from "lucide-react"
...
export function CatalogOverview() {
  const [query,setQuery]=useState(''),[activated,setActivated]=useState(false),[title,setTitle]=useState('二次函数专题练习')
  ...
  return <div className="prism-content">
    <header className="prism-page-heading flex items-center justify-between gap-6">
      <div><div className="mb-3 text-ui-hint  text-muted-foreground">智能曜彩 · INTELLIGENCE PRISM</div><h1>组件总览</h1><p>清晰的内容，有序的交互。为教学、研读与复核构建同一套界面语言。</p></div><div className="flex flex-wrap items-center gap-4"><Button variant="outline" render={<Link href="/next/reading"/>}><BookOpen/>材料研读</Button><BrandMark/></div>
    </header>
    ...
    {!query.trim()&&<section className="prism-preview-grid" aria-label="组件交互预览">
      <Card render={<article/>} className="prism-preview-tile"><div className="prism-preview-stage"><p className="text-ui-hint text-muted-foreground">一个明确的主操作</p><div className="flex flex-wrap items-center gap-2"><Button onClick={()=>setActivated(true)}><MousePointerClick/>触发示例操作</Button><Button variant="outline" onClick={()=>setActivated(false)}>重置</Button></div><p className="text-ui-hint text-muted-foreground" role="status">{activated?'已触发示例操作；未保存任何数据。':'点击按钮，体验操作与反馈。'}</p></div><Link href="/next/components/button">Button 按钮<ArrowUpRight/></Link></Card>
      <Card render={<article/>} className="prism-preview-tile"><div className="prism-preview-stage"><p className="text-ui-hint text-muted-foreground">标签、输入与说明各有位置</p><div className="space-y-2"><Label htmlFor="overview-title">练习名称</Label><Input id="overview-title" value={title} onChange={e=>setTitle(e.target.value)} maxLength={40}/><p className="text-ui-hint text-muted-foreground">用于学生端展示，可直接修改。</p></div></div><Link href="/next/components/form">Form 表单<ArrowUpRight/></Link></Card>
      <Card render={<article/>} className="prism-preview-tile"><div className="prism-preview-stage"><p className="text-ui-hint text-muted-foreground">标签说明作答形式</p><div className="flex flex-wrap gap-2"><QuestionTypeLabel label="单选" tone="blue"/><QuestionTypeLabel label="解答" tone="magenta"/><QuestionTypeLabel label="复合" tone="lime"/></div><div className="flex flex-wrap items-center gap-2"><span className="text-ui-action">函数与几何综合</span><QuestionPoints points={16} tone="lime"/></div></div><Link href="/next/components/question">Question 题目<ArrowUpRight/></Link></Card>
    </section>}
    <div className={query.trim()?'mt-8':''}>{componentGroups.map(group=>{
      ...
      return <section key={group.id} className="mb-9" aria-labelledby={'group-'+group.id}>
        <h2 id={'group-'+group.id} className="mb-3 flex items-center gap-2 text-section-title">{group.title}<span className="text-ui-hint text-muted-foreground">{group.id==='agent'?`${items.length} 组组件 · 骨架与示例另计`:items.length}</span></h2>
