...
export function CatalogOverview() {
  ...
  return <div className="prism-content">
    <div className="prism-page-heading flex flex-wrap items-start justify-between gap-5">
      ...
      <Button variant="outline" render={<Link href="/next/reading"/>}><BookOpen/>材料研读</Button>
    </div>
    <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
      ...
      <p className="text-ui-hint text-muted-foreground" aria-live="polite">{query?`找到 ${count} 项组件${relatedPages.length?` · ${relatedPages.length} 项骨架与示例`:''}`:`${components.filter(item=>!item.kind).length} 项基础组件 · ${components.filter(item=>item.kind==="pattern").length} 项业务与组合组件 · ${components.filter(item=>item.kind==="extension").length} 项扩展 · 3 套主题`}</p>
    </div>
    {componentGroups.map(group=>{
      ...
      return <section key={group.id} className="mb-9" aria-labelledby={'group-'+group.id}>
        <h2 id={'group-'+group.id} className="mb-3 flex scroll-mt-20 flex-wrap items-center gap-2 text-section-title">{group.title}<span className="text-ui-hint text-muted-foreground">{group.id==='agent'?`${items.length} 组组件 · 骨架与示例另计`:items.length}</span></h2>
        <div className="grid grid-cols-1 gap-x-8 md:grid-cols-2 xl:grid-cols-3">
          {items.map(({item,href})=><Link key={item.id} href={href} className="group min-w-0 border-b py-4">
