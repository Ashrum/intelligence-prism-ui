...
import { previewDraft, type MathRenderer } from '@/lib/prism-next/draft-math'
...
export function DraftMathPreview({value,label='当前题干预览'}:{value:string;label?:string}) {
 ...
 const id=useId(),parts=useMemo(()=>renderer?previewDraft(value,renderer):[{kind:'text' as const,source:value}],[value,renderer])
 ...
 return <section className="prism-draft-preview min-w-0 space-y-3 rounded-lg bg-secondary p-4" aria-labelledby={id}>
  ...
  {!!errors.length&&<div role="status" className="space-y-1 text-ui-hint text-warning-foreground">{errors.map(error=><p key={error}>{error}</p>)}</div>}
  <p className="text-ui-hint text-muted-foreground">仅预览排版，不校验数学结论。未识别内容按原文显示。</p>
  <details className="text-ui-hint text-muted-foreground"><summary className="cursor-pointer text-ui-action">复杂公式怎么输入</summary><p className="mt-2">{'常见代数式可直接输入；复杂公式使用 \\(…\\) 或 \\[ … \\]，例如 \\(\\frac{1}{\\sqrt{x^2+1}}\\)。'}</p></details>
 </section>
}
