      ...
      <DialogPopup className="max-w-4xl h-[min(45rem,calc(100dvh-3rem))]" closeProps={{ "aria-label": "取消并关闭选择器" }} finalFocus={() => returnFocus.current?.isConnected ? returnFocus.current : triggerRef.current}>
        ...
        <div className="flex min-h-0 flex-1 flex-col px-4 sm:px-6">
          <div className="flex shrink-0 flex-wrap items-end gap-3 pb-4">
            <div className="w-full max-w-72"><Label htmlFor={`${controlId}-popup-book`}>教材</Label><Select items={bookOptions} value={book.id} onValueChange={value => { if (value) { setBookId(value); setView("browse") } }}><SelectTrigger id={`${controlId}-popup-book`}><SelectValue /></SelectTrigger><SelectPopup>{bookOptions.map(item => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</SelectPopup></Select></div>
...
function DirectoryChoices({ data, kind, session, onSessionChange, checkedIds, onCheckedChange }: {
  ...
  function row(id: string, depth: number): React.ReactNode {
    ...
    return <div key={id}>
      <div ref={session.targetId === id ? targetRef : undefined} tabIndex={session.targetId === id ? -1 : undefined} className={cn("flex min-w-0 items-start gap-2 rounded-md px-2 py-2 outline-none focus:ring-2 focus:ring-ring", session.targetId === id && "bg-accent text-accent-foreground", folder && "mt-3 first:mt-0")} style={{ marginInlineStart: Math.min(depth, 2) * 12 }}>
        ...
        <div className="min-w-0 flex-1"><Label htmlFor={`${inputId}-${id}`} className={cn("block w-full cursor-pointer break-words text-left text-sm font-normal leading-6", folder && "font-medium")}>{node.code && <span className="mr-2 font-normal text-muted-foreground prism-numeric">{node.code}</span>}{node.title}</Label>{folder && <p className="text-xs leading-5 text-muted-foreground">已选 {countIn(id)} / {directoryLeaves(data, id).length} {unit(kind)}</p>}</div>
      </div>
      {node.children.map(child => row(child, depth + 1))}
    </div>
  }
  return <div className="flex h-full min-h-0 flex-col">
    <div className="shrink-0"><Label htmlFor={inputId}>搜索当前教材的{kindName(kind)}</Label><InputGroup><InputGroupAddon><Search /></InputGroupAddon><InputGroupInput ref={inputRef} id={inputId} value={query} onChange={event => onSessionChange({ query: event.target.value, targetId: undefined })} placeholder="搜索名称或编号，无需逐层查找" />{query && <InputGroupAddon align="inline-end"><Button size="icon-xs" variant="ghost" aria-label="清除范围搜索" onClick={clearSearch}><X /></Button></InputGroupAddon>}</InputGroup></div>
