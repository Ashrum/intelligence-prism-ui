...
export function TextbookRangePicker({ textbooks, selections, onSelectionsChange, compactTrigger = false, triggerLabel = "选择范围" }: {
  ...
  if (!book) return <p className="text-ui-hint text-muted-foreground">暂无可用教材。</p>
  ...
  return <div className="min-w-0" data-range-picker>
    <Dialog open={open} onOpenChange={next => { if (next) initialize(); setOpen(next) }}>
      ...
      <section className="mt-6" aria-label="已应用范围">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><h3 className="text-item-title">已应用范围</h3><p className="mt-1 text-ui-hint text-muted-foreground" data-applied-count>{countLabel(textbooks, selections)}</p></div>{hasApplied && <Button size="sm" variant="ghost" onClick={event => launch("selected", event.currentTarget)}>查看全部与调整</Button>}</div>
        {hasApplied ? <SelectionSummary textbooks={textbooks} selections={selections} compact onRemove={removeApplied} onAdjust={(context, source) => launch("browse", source, context)} onShowAll={source => launch("selected", source)} /> : <p className="rounded-lg bg-muted/50 px-4 py-5 text-ui-hint text-muted-foreground">尚未选择范围。选择后，这里会保留具体名称和来源，方便随时调整。</p>}
        <div className="mt-2 flex min-h-7 items-center gap-2 text-ui-hint text-muted-foreground" role="status">{status}{undo && <Button size="sm" variant="ghost" onClick={() => { onSelectionsChange(previous => ({ ...previous, [undo.scope]: [...new Set([...(previous[undo.scope] ?? []), ...undo.ids])] })); setUndo(null); setStatus("已恢复移除的内容。") }}>撤销</Button>}</div>
      </section></>}
      <DialogPopup className="max-w-4xl h-[min(45rem,calc(100dvh-3rem))]" closeProps={{ "aria-label": "取消并关闭选择器" }} finalFocus={() => returnFocus.current?.isConnected ? returnFocus.current : triggerRef.current}>
        ...
        <DialogFooter className="mt-3 shrink-0 flex-row flex-wrap items-center justify-between gap-3 sm:justify-between"><div className="text-ui-body" role="status"><span className="mr-2 text-muted-foreground">待应用</span><span className="font-medium">{countLabel(textbooks, draft)}</span></div><div className="flex gap-2"><Button variant="outline" onClick={() => setOpen(false)}>取消</Button><Button onClick={() => { onSelectionsChange(copySelections(draft)); setStatus("选择已应用。"); setOpen(false) }}><Check />应用选择</Button></div></DialogFooter>
      </DialogPopup>
    </Dialog>
  </div>
}
...
function SelectionSummary({ textbooks, selections, compact = false, onRemove, onAdjust, onShowAll }: {
  ...
  const groups = selectionGroups(textbooks, selections)
  if (!groups.length) return <p className="py-8 text-ui-hint text-muted-foreground">尚未选择内容，可返回目录继续选择。</p>
  return <div className="space-y-5">{groups.map(group => {
    ...
    return <section key={group.scope} aria-label={`${group.book.title} / ${kindName(group.kind)}选择摘要`}>
      <h4 className="flex flex-wrap items-center gap-x-3 gap-y-1 text-item-title">{group.book.title}<span className="font-normal text-muted-foreground">{kindName(group.kind)} · {group.ids.length} {unit(group.kind)}</span></h4>
      <ul className="mt-2 space-y-1">{(compact ? entries.slice(0, 3) : entries).map(entry => {
        ...
        return <li key={entry.id} className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1 rounded-md bg-muted/45 px-3 py-2">
          <div className="min-w-0 flex-[1_1_12rem]"><p className="break-words text-ui-hint">{node.title}{isGroup && <span className="ml-2 whitespace-nowrap text-ui-hint text-muted-foreground">全部 {entry.leafIds.length} {unit(group.kind)}</span>}</p>{parentPath && <p className="break-words text-ui-hint text-muted-foreground">{parentPath}</p>}</div>
  ...
  function row(id: string): React.ReactNode {
    ...
    return <div key={id}>
      <div ref={session.targetId === id ? targetRef : undefined} tabIndex={session.targetId === id ? -1 : undefined} data-directory-level={data.paths[id].length} className={cn("flex min-w-0 items-start gap-2 rounded-md px-2 py-2 outline-none focus:ring-2 focus:ring-ring", session.targetId === id && "bg-accent text-accent-foreground", folder && "mt-2 first:mt-0")}>
        ...
        <div className="min-w-0 flex-1"><Label htmlFor={`${inputId}-${id}`} className={cn("block w-full cursor-pointer break-words text-left text-ui-hint", folder && "font-medium")}>{node.code && <span className="mr-2 font-normal text-muted-foreground prism-numeric">{node.code}</span>}{node.title}</Label>{folder && <p className="text-ui-hint text-muted-foreground">分组 · 已选 {countIn(id)} / {directoryLeaves(data, id).length} {unit(kind)}</p>}</div>
        <span className="shrink-0 pt-0.5 text-ui-hint text-muted-foreground">第 {data.paths[id].length} 级</span>
      </div>
      {folder && <div className="ml-4 border-l border-border/60 pl-1 sm:ml-5">{node.children.map(child => row(child))}</div>}
    </div>
  }
  return <div className="flex h-full min-h-0 flex-col">
    <p className="mb-3 shrink-0 text-ui-hint text-muted-foreground">当前{kindName(kind)}最深 {maxDepth} 级 · 首层为第 1 级 · 分组勾选包含全部下级</p>
    ...
    {projection.normalized ? <div className="mt-3 min-h-0 flex-1 overflow-y-auto overscroll-contain pb-3" aria-label="范围搜索结果">
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2"><p className="text-ui-hint text-muted-foreground" role="status">{matchingLeaves.length} 个可选项 · {matchingFolders.length} 个分组</p><Button size="sm" variant="outline" disabled={!matchingLeaves.length} onClick={() => onCheckedChange(previous => [...new Set([...previous, ...matchingLeaves])])}>选择搜索结果（{matchingLeaves.length}）</Button></div>
      <p className="mb-3 text-ui-hint text-muted-foreground">只批量选择下方命中的具体项，其他已选内容保持不变。</p>
      {matchingLeaves.map(id => <div key={id} className="flex min-w-0 items-start gap-2 rounded-md px-2 py-2 hover:bg-muted/50"><Checkbox className="mt-1" aria-label={`选择搜索结果${data.nodes[id].title}`} checked={checked.has(id)} onCheckedChange={() => { void selectionTree.getItemInstance(id).toggleCheckedState() }} /><div className="min-w-0"><p className="break-words text-ui-hint">{data.nodes[id].title}</p><p className="break-words text-ui-hint text-muted-foreground">{data.paths[id].slice(0, -1).map(parent => data.nodes[parent].title).join(" / ")}</p></div></div>)}
      {matchingFolders.map(id => <div key={id} className="flex min-w-0 items-center gap-2 px-2 py-2"><div className="min-w-0 flex-1"><p className="break-words text-ui-hint">{data.nodes[id].title}</p><p className="break-words text-ui-hint text-muted-foreground">分组 · {data.paths[id].slice(0, -1).map(parent => data.nodes[parent].title).join(" / ") || kindName(kind)}</p></div><Button size="sm" variant="ghost" onClick={() => onSessionChange({ chapterId: data.paths[id][0], query: "", targetId: id })}>进入分组<ChevronRight /></Button></div>)}
      {!matchingLeaves.length && !matchingFolders.length && <div className="py-8 text-center"><p className="mb-3 text-ui-hint text-muted-foreground">没有匹配项，已选内容仍然保留。</p><Button size="sm" variant="outline" onClick={clearSearch}>清除搜索</Button></div>}
    </div> : chapterId ? <div className="mt-3 grid min-h-0 flex-1 grid-cols-1 gap-3 sm:grid-cols-[13rem_minmax(0,1fr)]">
      <nav aria-label="章节定位" className="hidden min-h-0 space-y-1 overflow-y-auto overscroll-contain border-r pr-3 pb-3 sm:block">{chapterIds.map(id => <Button key={id} variant="ghost" className={cn("h-auto sm:h-auto min-h-10 w-full justify-start whitespace-normal px-3 py-2 text-left", chapterId === id && "bg-accent text-accent-foreground")} aria-current={chapterId === id ? "location" : undefined} onClick={() => onSessionChange({ chapterId: id, targetId: undefined })}><span className="min-w-0 flex-1"><span className="block break-words text-ui-hint">{data.nodes[id].code && <span className="mr-2 text-muted-foreground">{data.nodes[id].code}</span>}{data.nodes[id].title}</span><span className="block text-ui-hint text-muted-foreground">已选 {countIn(id)} / {directoryLeaves(data, id).length}</span></span><ChevronRight className="shrink-0" /></Button>)}</nav>
    ...
    </div> : <p className="py-8 text-ui-hint text-muted-foreground">此教材尚未设置目录。</p>}
  </div>
}
