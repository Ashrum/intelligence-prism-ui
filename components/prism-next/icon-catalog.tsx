...
export function IconCatalogDemo() {
  ...
  return <DemoSection title="图标语义目录" description="Lucide 线性图标；保持 coss 原有控件尺寸。中文名称与业务含义一起检索。">
    <div className="space-y-6">
      ...
      {!visible.length&&<p className="py-8 text-sm text-muted-foreground">没有匹配的图标。试试“教材”“目标”或英文名称。</p>}
      <section className="space-y-4 border-t pt-5" aria-label="所选图标规范"><div className="flex flex-wrap items-center gap-3"><SelectedIcon size={20} aria-hidden="true"/><strong className="text-sm">{selected.label}</strong><Badge variant="outline">{selected.group}</Badge><code className="text-sm">{selected.name}</code></div><p className="text-sm text-muted-foreground">{selected.meaning}。</p><div className="flex flex-wrap items-center gap-6">{[16,20,24].map(size=><div key={size} className="flex items-center gap-2 text-xs text-muted-foreground"><SelectedIcon size={size} strokeWidth={2} aria-hidden="true"/>{size} px</div>)}<Button variant="outline" onClick={async()=>{try{await navigator.clipboard.writeText(`import { ${selected.name}Icon } from "lucide-react"`);setMessage("已复制导入代码")}catch{setMessage("未能复制，请选择下方代码复制")}}}><CopyIcon/>复制导入</Button></div><code className="block overflow-x-auto rounded-md bg-muted p-3 text-xs">{`import { ${selected.name}Icon } from "lucide-react"`}</code><p className="min-h-5 text-sm text-muted-foreground" role="status">{message||"图标继承文字颜色。状态同时给出文字；独立图标按钮必须有可读名称。"}</p></section>
    </div>
  </DemoSection>
}
