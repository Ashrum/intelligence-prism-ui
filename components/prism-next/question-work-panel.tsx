...
export function QuestionWorkPanel({id,open,title,description,onClose,children,footer,footerVariant="default",headerActions,className="",scrollMemory,position="right",initialFocus,finalFocus,showCloseButton=true}:{id:string;open:boolean;title:string;description:string;onClose:()=>void;children:ReactNode;footer?:ReactNode;footerVariant?:ComponentProps<typeof DrawerFooter>["variant"];headerActions?:ReactNode;className?:string;scrollMemory?:RefObject<number>;position?:"right"|"bottom";initialFocus?:ComponentProps<typeof DrawerPopup>["initialFocus"];finalFocus?:ComponentProps<typeof DrawerPopup>["finalFocus"];showCloseButton?:boolean}) {
  ...
  return <Drawer position={position} open={open} modal={false} disablePointerDismissal onOpenChange={value=>{if(!value)onClose()}} onOpenChangeComplete={value=>{if(value)restoreScroll()}}>
    <DrawerPopup ref={popup} id={id} className={`q-task-drawer q-no-print motion-reduce:transition-none! ${className}`} portalProps={{className:"q-task-drawer-portal"}} initialFocus={initialFocus} finalFocus={finalFocus??(()=>document.querySelector<HTMLButtonElement>(`button[aria-controls="${id}"]`)??false)} onScrollCapture={event=>{const target=event.target as HTMLElement;if(scrollMemory&&target.dataset.slot==="scroll-area-viewport")scrollMemory.current=target.scrollTop}}>
      <DrawerHeader allowSelection>{headerActions?<div className={`flex flex-wrap items-center justify-between gap-2 ${showCloseButton?"pe-8":""}`}><DrawerTitle>{title}</DrawerTitle>{headerActions}</div>:<DrawerTitle>{title}</DrawerTitle>}<DrawerDescription>{description}</DrawerDescription></DrawerHeader>
