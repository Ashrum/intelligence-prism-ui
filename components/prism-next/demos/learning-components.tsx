...
import { QuestionReviewDemo } from "./question-review"
...
return <DemoSection title="可独立使用的业务组件" description="展示数据、状态与操作由外部传入；这里的实例不连接学习工作流。"><div className="mb-6 flex flex-wrap gap-3"><Button variant="outline" onClick={()=>{setSecond(!second);setResults({});setNote("");setSelected(undefined)}}>切换另一组数据</Button>{target&&<Button variant="ghost" render={<Link href={target}/>}>查看应用示例</Button>}</div>
{kind==="evaluation"&&(second?<QuestionReview key="short" question={shortQuestion} attempts={{"1":"周二，下午六点。"}} initialScores={{score:4}} learner="样本 B"/>:<QuestionReviewDemo key="math"/>)}
{kind==="diagnosis"&&<DiagnosisEvidenceTable items={[{id:"observation-1",title,observation:"需要根据现有证据进一步核对。",source:second?"阅读记录 B":"作答记录 A",location:second?"信息提取":"数量关系",status:<StatusBadge tone="pending">待确认</StatusBadge>}]} onInspect={setSelected}/>}
{kind==="goals"&&<LearningGoalCard id={second?"goal-b":"goal-a"} title={title} status={<StatusBadge tone="pending">待核验</StatusBadge>} source={second?"来源：阅读记录 B":"来源：作答记录 A"} actions={<Button variant="outline" onClick={()=>setSelected("goal")}>查看依据</Button>}><VerificationFields criteria={criteria} results={results} note={note} onResultsChange={setResults} onNoteChange={setNote}/></LearningGoalCard>}
{kind==="learning-plan"&&<LearningTaskList items={[{id:"task-1",title,condition:"完成后保留过程记录。",schedule:second?"2026-09-18 · 20 分钟":"2026-09-16 · 15 分钟",status:<StatusBadge tone="pending">待开始</StatusBadge>,actions:<Button onClick={()=>setNotice("外部收到开始任务的请求。")}>开始</Button>} ]}/>}
