...
return <>{kind==="agent-components"&&<div className="mb-12"><AgentContentInputDemo/><AgentReviewQueueDemo/><AgentObjectPickerDemo/><AgentMetricSummaryDemo/><AgentScopeBuilderDemo/><AgentObjectViewerDemo/><AgentCollectionBasketDemo/><AgentFileInputDemo/><AgentDocumentWorkspaceDemo/><AgentItemReviewerDemo/><AgentEvidenceDrilldownDemo/><AgentExceptionHandlerDemo/><AgentRecordViewsDemo/><AgentChangeSetDemo/><AgentSemanticGroupDemo/></div>}<DemoSection title={kind==="agent-components"?"既有输入与引导组件":"可独立使用的业务组件"} description="展示数据、状态与操作由外部传入；这里的实例不连接学习工作流。"><div className="mb-6 flex flex-wrap gap-3"><Button variant="outline" onClick={()=>{setSecond(!second);setResults({});setNote("");setSelected(undefined)}}>切换另一组数据</Button>{target&&<Button variant="ghost" render={<Link href={target}/>}>查看应用示例</Button>}</div>
