...
export function LearningComponentDemo({kind}:{kind:string}){const panelId=useId();const [second,setSecond]=useState(false),[selected,setSelected]=useState<string>(),[results,setResults]=useState<Record<string,CriterionResult|undefined>>({}),[note,setNote]=useState(""),[date,setDate]=useState(new Date(2026,8,16)),[month,setMonth]=useState(new Date(2026,8,1)),[prompt,setPrompt]=useState(""),[running,setRunning]=useState(false),[notice,setNotice]=useState("");const title=second?"说明判断依据":"明确数量与单位";const criteria=[{id:"evidence",text:second?"引用材料中的具体事实":"说明数值所代表的含义"},{id:"reason",text:second?"解释事实与结论的关系":"准确标明单位与范围"}];const target=kind==="agent-components"?"/next/agent":`/next/examples/${kind}`
return <DemoSection title="可独立使用的业务组件" description="展示数据、状态与操作由外部传入；这里的实例不连接学习工作流。"><div className="mb-6 flex flex-wrap gap-3"><Button variant="outline" onClick={()=>{setSecond(!second);setResults({});setNote("");setSelected(undefined)}}>切换另一组数据</Button><Button variant="ghost" render={<Link href={target}/>}>查看应用示例</Button></div>
