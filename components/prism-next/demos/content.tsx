...
export function CardDemo(){
 ...
 return <>
  <DemoSection title="内容与操作" description="标题、说明、正文和底部操作共同围绕一个对象。">
   <Card className="max-w-sm"><CardHeader><CardTitle>二次方程的实数根</CardTitle><CardDescription>数学 · 函数与方程</CardDescription></CardHeader><CardPanel><p className="text-read-body">从判别式开始，理解两实根、重根与无实根的区别。</p><div className="mt-4 flex items-center gap-3 text-ui-hint text-muted-foreground"><Badge variant="outline">待复核</Badge><span>预计 8 分钟</span></div></CardPanel><CardFooter className="gap-2"><Button render={<Link href="/next/reading"/>}>阅读材料</Button><Button variant="ghost" onClick={()=>setSaved(v=>!v)} aria-pressed={saved}>{saved?'已收藏':'收藏'}</Button></CardFooter></Card>
  </DemoSection>
  <DemoSection title="紧凑横向卡片" description="用于文件、资源或清单中的单个条目。整卡只有一个链接，不嵌套其他操作。">
   <Card render={<Link href="/next/reading"/>} className="max-w-xl transition-colors hover:bg-muted/40 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring"><CardPanel className="flex items-center gap-4 p-4"><FileText className="size-6 shrink-0 text-muted-foreground"/><div className="min-w-0 flex-1"><h3 className="text-item-title">函数图像阅读材料</h3><p className="mt-1 text-ui-hint text-muted-foreground">例题讲解 · 预计 8 分钟</p></div><ChevronDown className="size-4 -rotate-90 text-muted-foreground"/></CardPanel></Card>
  </DemoSection>
  <DemoSection title="指标摘要" description="突出一项数值；补充口径与状态，避免把整张卡片染色。">
   <Card className="max-w-sm"><CardHeader><CardTitle className="text-block-title">已完成复核</CardTitle><CardDescription>本周 · 示例数据</CardDescription><CardAction><Badge variant="outline">持续更新</Badge></CardAction></CardHeader><CardPanel><p className="text-stat-display tabular-nums">128<span className="ml-2 text-ui-hint text-muted-foreground">份</span></p><p className="mt-3 text-ui-hint text-muted-foreground">共 160 份材料，完成率 80%。</p></CardPanel></Card>
  </DemoSection>
  <DemoSection title="人物资料" description="头像、身份与附属信息组成一张紧凑资料卡。">
   <Card className="max-w-sm"><CardPanel><div className="flex items-center gap-4"><Avatar className="size-12 text-read-body"><AvatarFallback>陈</AvatarFallback></Avatar><div><h3 className="font-semibold">陈老师</h3><p className="mt-1 text-ui-hint text-muted-foreground">数学 · 教研成员</p></div></div><p className="mt-5 text-ui-hint text-muted-foreground">关注函数与几何教学，参与材料整理和复核。</p></CardPanel></Card>
  </DemoSection>
  <DemoSection title="可选择卡片" description="整卡可选，单选标记与边框共同表达当前选择；支持键盘切换。">
   <RadioGroup value={selected} onValueChange={setSelected} aria-label="卡片展示方案" className="grid max-w-2xl gap-3 sm:grid-cols-2">{[{id:"brief",title:"摘要模式",text:"优先显示标题、状态和关键数据。"},{id:"detail",title:"详细模式",text:"显示说明、关联内容与操作。"}].map(item=><Card key={item.id} render={<label/>} className={selected===item.id?"cursor-pointer border-foreground":"cursor-pointer hover:border-muted-foreground"}><CardPanel className="flex items-start gap-3 p-5"><Radio value={item.id} aria-label={item.title} className="mt-0.5"/><div><h3 className="text-item-title">{item.title}</h3><p className="mt-2 text-ui-hint text-muted-foreground">{item.text}</p></div></CardPanel></Card>)}</RadioGroup><Feedback>当前选择：{selected==="brief"?"摘要模式":"详细模式"}</Feedback>
  </DemoSection>
  <DemoSection title="带分组标题的卡片" description="使用 CardFrame 组织同组内容，分组说明留在外层，正文留在卡片内。">
   <CardFrame className="max-w-lg"><CardFrameHeader><CardFrameTitle>本周材料</CardFrameTitle><CardFrameDescription>待完成的阅读与复核</CardFrameDescription></CardFrameHeader><Card><CardHeader><CardTitle>函数的单调性</CardTitle><CardDescription>知识梳理 · 已更新</CardDescription></CardHeader><CardPanel><p className="text-ui-hint">从图像观察变化，再用定义说明结论。</p></CardPanel><CardFooter><Button variant="outline" render={<Link href="/next/reading"/>}>查看材料</Button></CardFooter></Card></CardFrame>
  </DemoSection>
 </>
}
