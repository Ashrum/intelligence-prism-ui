...
function SaveDialogParticle() {
  ...
  return <DemoSection title="提交等待、失败保留与关闭确认" description="开启失败演示后首次保存会失败；重试使用原输入，未保存时关闭会先确认。" sources={["p-dialog-4"]}>
    ...
    <Dialog open={open} onOpenChange={changeOpen}><DialogTrigger ref={triggerRef} render={<Button variant="outline" />}>编辑修订备注</DialogTrigger><DialogPopup closeProps={{ "aria-label": "关闭修订备注", disabled: pending }} finalFocus={triggerRef}><DialogHeader><DialogTitle>编辑修订备注</DialogTitle><DialogDescription>保存结果确认前，当前输入会保留在此处。</DialogDescription></DialogHeader><Form className="contents" noValidate onSubmit={event => { event.preventDefault(); submit() }}><DialogPanel><Label htmlFor="particle-save-notes">修订备注</Label><Textarea id="particle-save-notes" ref={inputRef} value={draft} onChange={event => { setDraft(event.target.value); setError("") }} readOnly={pending} aria-invalid={!!error && !draft.trim()} aria-describedby="particle-save-feedback" maxLength={500} /><p className="mt-2 text-right text-xs text-muted-foreground prism-numeric">{draft.length} / 500</p><div id="particle-save-feedback" className="mt-3 min-h-6 text-sm">{error ? <p role="alert" className="text-destructive-foreground">{error}</p> : <p role="status" className="text-muted-foreground">{pending ? "正在保存，请稍候……" : "本页为交互演示，不向服务器发送数据。"}</p>}</div></DialogPanel><DialogFooter><DialogClose render={<Button variant="ghost" disabled={pending} />}>取消</DialogClose><Button type="submit" loading={pending}>{error && draft.trim() ? "重试保存" : "保存备注"}</Button></DialogFooter></Form></DialogPopup>
