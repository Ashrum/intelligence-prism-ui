...
export function QuestionComponentDemo(){const [id,setId]=useState(samples[0].id);return <DemoSection title="独立题目卡片" description="题目数据、详情、作答与操作分别传入。同一组件用于选题、编排、作答和复核。"><div className="mb-6 flex flex-wrap items-center justify-between gap-3"><QuestionSelect label="题型样本" value={id} onChange={setId} items={samples.map(q=>({value:q.id,label:`${q.kind} · ${q.title}`}))}/><Button variant="outline" render={<Link href="/next/examples/questions"/>}>题库与组卷应用示例</Button></div><QuestionSpecimen key={id} id={id}/><PrintMetadataDemo id={id}/><details className="mt-6 text-ui-body"><summary className="cursor-pointer text-muted-foreground">使用接口</summary><pre className="mt-3 overflow-x-auto rounded-lg bg-muted p-4 text-ui-hint">{'<QuestionCard question={question} />\n<QuestionCard question={question} checked={selected}\n  onCheckedChange={setSelected} actions={actions}\n  details={details} detailsOpen={open}\n  onDetailsOpenChange={setOpen} />'}</pre></details></DemoSection>}
function PrintMetadataDemo({id}:{id:string}) {
 const question=samples.find(q=>q.id===id)!
 const [showIds,setShowIds]=useState(true),[preferences,setPreferences]=useState(createPrintPreferences),[settingsOpen,setSettingsOpen]=useState(false)
 return <section id="print-metadata" className="mt-8 space-y-4"><h3 className="text-block-title">打印题目标识</h3><Button variant="outline" aria-pressed={showIds} onClick={()=>setShowIds(v=>!v)}>题号与版本行：{showIds?'显示':'隐藏'}</Button><QuestionPrint entries={[{id:question.id,points:question.points}]} questions={[question]} versions={{[question.id]:'1.0'}} title="打印元数据示例" minutes={30} blocked={false} showQuestionIds={showIds} preferences={preferences} onPreferencesChange={setPreferences} settingsOpen={settingsOpen} onSettingsChange={setSettingsOpen}/></section>
}
