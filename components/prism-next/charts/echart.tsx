...
export function EChart({option,label,height=300,onSelect}:{option:EChartsOption;label:string;height?:number;onSelect?:(id:string,seriesId?:string)=>void}){
 ...
 return <div className="min-w-0"><div ref={node} role="img" aria-label={label} style={{height,width:"100%"}} data-chart-engine="echarts"/>{error&&<p role="alert" className="text-ui-body text-destructive-foreground">图形未能加载，请查看下方数据。</p>}</div>
}
