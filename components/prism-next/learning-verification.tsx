...
export function VerificationPanel({ goal, onClose, active }: { goal: Goal; onClose: () => void; active: boolean }) {
  ...
  return <QuestionWorkPanel id="learning-verification" open={active} title="核验目标" description={`${goal.id} · 标准 v${goal.version}。两份独立新作答均满足所有标准，才判定达成。`} onClose={onClose} footer={<><Button onClick={save}>保存核验</Button><Button variant="outline" onClick={onClose}>取消</Button></>}>
