...
import { createReviewEditor, initialReviewScores, reviewAttempts, type ReviewEditor } from "@/lib/prism-next/question-review-model"
...
const initial=initialReviewScores
const limits=Object.fromEntries(criteria.map(item=>[item.id,item.points]))
const attempts=reviewAttempts
export function QuestionReview({editor,onEditorChange,onConfirm}:{editor?:ReviewEditor;onEditorChange?:Dispatch<SetStateAction<ReviewEditor>>;onConfirm?:(scores:Record<string,number>,reason:string)=>void}={}) {
  const [local,setLocal]=useState(createReviewEditor)
  const {scores,saved,reason,record,error}=editor??local
  const change=onEditorChange??setLocal
  ...
  return <div className="space-y-6">
    ...
    <div className="flex flex-wrap gap-5 rounded-lg bg-muted/50 p-4 text-sm"><span>初评得分 <strong className="tabular-nums">10 / 16</strong></span><span>已记录得分 <strong className="tabular-nums">{Object.values(saved).reduce((a,b)=>a+b,0)} / 16</strong></span>{view==="review"&&<span>复核中得分 <strong className="tabular-nums">{Object.values(scores).some(value=>value===null)?`已评 ${total} 分 · 有待评分项`:`${total} / 16`}</strong></span>}</div>
    ...
    {view==="review"&&<section className="space-y-4 rounded-lg bg-muted/50 p-5"><div className="space-y-2"><Label htmlFor="question-review-reason">复核理由{criteria.some(item=>scores[item.id]!==saved[item.id])?"（调整分数后必填）":""}</Label><Textarea id="question-review-reason" value={reason} onChange={event=>setReason(event.target.value)} placeholder="说明调整的评分点与依据。"/></div>{error&&<p role="alert" className="text-sm text-destructive">{error}</p>}<div className="flex flex-wrap gap-2"><Button disabled={!dirty&&!!record} onClick={()=>{const message=reviewError(scores,limits,saved,reason);setError(message);if(!message){change(previous=>({...previous,saved:{...scores} as Record<string,number>,record:reason.trim()||"复核确认，维持各评分点原分数。",reason:""}));onConfirm?.(scores as Record<string,number>,reason)}}}>确认复核</Button><Button variant="outline" disabled={!dirty} onClick={()=>{setScores({...saved});setReason("");setError("")}}>取消修改</Button></div></section>}
