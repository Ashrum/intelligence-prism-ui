...
export function DemoSection({title,description,children,id,sources}:{title:string;description?:string;children:ReactNode;id?:string;sources?:string[]}) {
  return <section id={id} className="prism-demo-section"><header><h2>{title}</h2>{description&&<p>{description}</p>}{sources&&<div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-ui-hint text-muted-foreground">{sources.map(source=><a key={source} href={`https://github.com/cosscom/coss/blob/e937becd2d5ffb5c621eed6f8b1f223cbb6051e7/apps/ui/registry/default/particles/${source}.tsx`} target="_blank" rel="noreferrer" className="hover:text-foreground">coss · {source} ↗</a>)}</div>}</header><div className="prism-demo">{children}</div></section>
}
export function Feedback({children}:{children:ReactNode}) { return <p className="mt-4 min-h-6 text-ui-hint text-muted-foreground" role="status" aria-live="polite">{children}</p> }
