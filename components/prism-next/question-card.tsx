...
export function QuestionCard({question,number,details,detailsOpen,onDetailsOpenChange,checked,onCheckedChange,selectionDisabled=false,selectionLabel,actions,secondaryActions,headerActions,reading=false,compact=false,variant="card",status,header,showPoints=true,displayPoints,displayPartPoints,highlighted=false}:{
  ...
  actions?:ReactNode;secondaryActions?:ReactNode;headerActions?:ReactNode;reading?:boolean;compact?:boolean;variant?:"card"|"list"
  ...
  return <article data-question-id={question.id} data-highlight={highlighted||undefined} data-variant={variant} aria-labelledby={id} className={cn("prism-question min-w-0 text-foreground",variant==="list"?"py-6":"rounded-xl border bg-background px-4 py-5 sm:p-6",checked&&variant==="card"&&"border-primary/60",reading&&"border-transparent px-0 sm:px-0")}>
    <div className={cn("grid min-w-0 gap-x-4 sm:gap-x-5",hasRail?"grid-cols-[2rem_minmax(0,1fr)]":"grid-cols-1")}>
      ...
      <div className="min-w-0">
        <header className="mb-4">
          <div className="flex items-start justify-between gap-3"><div className="flex min-w-0 flex-1 items-start gap-2">{header}<h3 id={id} className={cn("min-w-0 flex-1 break-words font-semibold leading-7 text-(--heading)",variant==="list"?"text-base":"text-lg")}>{number!==undefined&&<span className="sr-only">第 {number} 题 · </span>}{question.title}</h3></div>{headerActions&&<Toolbar aria-label={`${question.title}快捷操作`} className="shrink-0 rounded-none border-0 bg-transparent p-0"><ToolbarGroup>{headerActions}</ToolbarGroup></Toolbar>}</div>
          <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-2"><Badge variant="outline" size="lg">{question.kind}</Badge>{showPoints&&<Badge variant="info" size="lg" className="tabular-nums">{displayPoints??question.points} 分</Badge>}{question.parts&&<span className="text-xs text-muted-foreground">整题 · 含 {question.parts.length} 个小问</span>}{status}</div>
        </header>
        {compact?<div className={cn("prism-question-copy min-w-0 text-base leading-[1.9] text-muted-foreground",variant!=="list"&&"line-clamp-2")}>{question.stem}</div>:<QuestionContent question={content}/>}
