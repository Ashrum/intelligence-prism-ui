...
export function QuestionPrint({entries,questions,versions,blocked,title,showPoints=true,description,minutes,preferences,onPreferencesChange,settingsOpen,onSettingsChange}:{settingsOpen:boolean;onSettingsChange:(open:boolean)=>void;preferences:PrintPreferences;onPreferencesChange:(update:(previous:PrintPreferences)=>PrintPreferences)=>void;entries:PaperEntry[];questions:QuestionRecord[];versions:Record<string,string>;blocked:boolean;title:string;showPoints?:boolean;description?:string;minutes:number}) {
  ...
  return <div className="question-print-preview space-y-5" style={styles}>
    ...
    <div className="q-print-workbench" data-settings={settingsOpen||undefined}>
      {settingsOpen&&<QuestionWorkPanel id="question-print-settings" scrollMemory={settingsScroll} title="排版设置" description="修改后即时更新纸面。显示比例仅影响屏幕预览。" onClose={()=>{keepAnchor();onSettingsChange(false)}} footer={<Button variant="outline" onClick={()=>{keepAnchor();onPreferencesChange(previous=>({...previous,margin:16,fontSize:12,spaces:{},breaks:[]}))}}>恢复默认</Button>}><div className="space-y-6"><div className="space-y-2"><Label className="block">页边距</Label><QuestionSelect label="页边距" value={String(margin)} items={[{value:"16",label:"标准 · 16 mm"},{value:"20",label:"宽边 · 20 mm"}]} onChange={value=>setMargin(Number(value))}/></div><div className="space-y-2"><Label className="block">正文字号</Label><QuestionSelect label="纸面正文字号" value={String(fontSize)} items={[{value:"11",label:"11 pt"},{value:"12",label:"12 pt"}]} onChange={value=>setFontSize(Number(value))}/></div>{answerFields.length>0&&(mode==="paper"||mode==="response")&&<section className="space-y-3"><h4 className="text-sm font-semibold">每小问作答高度</h4>{answerFields.map(field=><div key={field.id} className="flex items-center justify-between gap-3"><Label>{field.label}</Label><QuestionSelect label={`${field.label}作答高度`} value={String(spaces[field.id]??40)} items={[20,40,60,80].map(value=>({value:String(value),label:`${value} mm`}))} onChange={value=>{revealUnit(field.id.includes(":")?field.id.replace(":","-part-"):`${field.id}-stem`);setSpaces(previous=>({...previous,[field.id]:Number(value)}))}}/></div>)}</section>}<section className="space-y-3"><h4 className="text-sm font-semibold">指定题目另起一页</h4>{entries.map((entry,index)=><label key={entry.id} className="flex items-start gap-3 text-sm"><Checkbox checked={breaks.includes(entry.id)} onCheckedChange={checked=>{revealUnit(entry.id);setBreaks(previous=>checked?[...previous,entry.id]:previous.filter(id=>id!==entry.id))}}/><span>第 {index+1} 题 · {questions.find(question=>question.id===entry.id)?.title}</span></label>)}</section></div></QuestionWorkPanel>}
