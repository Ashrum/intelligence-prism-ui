...
export function TypographyFoundation(){
  ...
  return <div className="prism-content">
    ...
    <DemoSection id="type-audit" title="全站检查" description="2026-09-20 · 浏览器检查与源码检查分别记录；以下 93 页 / 279 次为当日检查记录，仅适用于已执行的页面、主题与状态，不代表所有交互状态均已通过。">
      <div className="grid gap-6 sm:grid-cols-3"><div><p className="text-ui-hint text-muted-foreground">页面入口</p><p className="mt-2 text-stat-display tabular-nums">{audit.pageCount}<span className="ml-1 text-ui-body">页</span></p></div><div><p className="text-ui-hint text-muted-foreground">三主题展示检查</p><p className="mt-2 text-stat-display tabular-nums">{audit.themeVisits}<span className="ml-1 text-ui-body">次</span></p></div><div><p className="text-ui-hint text-muted-foreground">历史已执行检查内未解决问题</p><p className="mt-2 text-stat-display tabular-nums">{audit.unresolved}<span className="ml-1 text-ui-body">项</span></p></div></div>
      <p className="mt-6 text-ui-hint">已检查全部 80 个组件页，以及基础规范、阅读、Agent、教师用例、页面骨架和应用示例。检查实际字号、明确角色的行高与字重、普通 DOM 文字的合成对比度及页面级横向溢出；当时未全面检查组件内部滚动。</p>
      <div className="mt-6 grid gap-6 lg:grid-cols-2"><section><h3 className="text-block-title">已操作的代表性状态</h3><ul className="mt-3 list-disc space-y-2 pl-5 text-ui-hint"><li>题干查看与编辑：均为 16 / 28 / 400。</li><li>执行中、部分完成、等待核对：均为 14 / 20 / 500。</li><li>短输入：1024px 视窗为 14 / 20，390px 视窗为 16 / 24。</li><li>长编辑区：两种视窗均为 16 / 28。</li><li>弹窗错误说明为 14 / 22；侧面板标题为 20 / 30，长编辑为 16 / 28。</li><li>解析工作区 390px 视窗：当日确认标题与题干字号保持、页面级无横向溢出；此结果不涵盖步骤条内部的信息可见性。</li></ul></section><section><h3 className="text-block-title">工程约束与验证边界</h3><p className="mt-3 text-ui-hint">93 个自有 TSX 文件纳入语义字号扫描。局部 CSS 字号也由构建检查约束；54 个固定 coss 组件源文件保持一致，现有回归测试通过。</p><p className="mt-3 text-ui-hint text-muted-foreground">MathML 上下标、SVG / Canvas 图中文字、打印纸面和禁用态单独处理，不计入普通 DOM 文字的通过结论。未穷举所有交互状态组合。</p><p className="mt-3 text-ui-hint text-warning-foreground">待真机确认：Windows / Apple 实际字形与字重区分、iOS Safari 聚焦表现。云端浏览器结果不代替真机结论。</p></section></div>
      <p className="mt-6 text-ui-hint">横向布局分别检查：页面意外溢出、关键步骤与操作是否被遮挡，以及表格、公式、原稿的必要局部滚动。局部 scrollWidth 大于 clientWidth 只作为复核线索，不直接判定违规；关键流程信息不能依赖不明显的滚动条才能发现。</p>
      <section className="mt-6"><h3 className="text-block-title">2026-09-21 · 定向复核</h3><p className="mt-3 text-ui-hint">本轮已执行检查未发现未解决问题：解析工作区 390px 下，摘要显示当前步骤 / 总步骤，五段指示与展开后的全部步骤无横向溢出；Enter / Space 可展开、收起，示例从 1/5 推进至 5/5 时摘要同步。浅色、暖纸、深色下摘要保持 14 / 20，步骤说明保持 14 / 22，题干编辑保持 16 / 28；1024px 下完整步骤条可见。</p><p className="mt-3 text-ui-hint text-muted-foreground">本次仅复核解析步骤与字体页检查表述，未重跑上述 93 页 / 279 次，也未扩展为所有交互状态的通过结论。</p></section>
