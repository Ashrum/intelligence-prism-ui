...
export function QuestionSelect({label,value,items,onChange,disabled=false}:{disabled?:boolean;label:string;value:string;items:{value:string;label:string}[];onChange:(value:string)=>void}) {
  return <Select disabled={disabled} items={items} value={value} onValueChange={value=>{if(value!==null)onChange(value)}}><SelectTrigger aria-label={label} className="w-fit max-w-full"><SelectValue/></SelectTrigger><SelectPopup>{items.map(item=><SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</SelectPopup></Select>
}
