...
export function PointsField({label,value,max=100,step=0.5,unit="分",onChange,invalid,describedBy}:{label:string;value:number|null;max?:number;step?:number;unit?:string;onChange:(value:number|null)=>void;invalid?:boolean;describedBy?:string}) {
  return <NumberField value={value} onValueChange={onChange} min={0} max={max} step={step} smallStep={step} snapOnStep onValueCommitted={next=>{if(next!==null)onChange(Math.round(next/step)*step)}} className="w-32 shrink-0"><NumberFieldGroup><NumberFieldDecrement aria-label={`${label}减少${unit}`}/><NumberFieldInput aria-label={label} aria-invalid={invalid||undefined} aria-describedby={describedBy}/><NumberFieldIncrement aria-label={`${label}增加${unit}`}/></NumberFieldGroup></NumberField>
}
