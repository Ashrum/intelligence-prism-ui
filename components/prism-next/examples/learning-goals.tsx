"use client"
import { LearningGoalCard } from "@/components/prism-next/learning-components"
...
export function LearningGoals({ go, active }: { go: (stage: Stage) => void; active: boolean }) {
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-start justify-between gap-4"><div className="space-y-1"><h3 className="text-block-title">把诊断变成可验证的目标</h3><p className="text-ui-hint text-muted-foreground">明确起点、达成标准、截止日期与验证方法。任务完成数单独记录。</p></div><Button variant="outline" disabled={!canCreate} onClick={create}>从已确认诊断创建</Button></div>
