...
export function LearningPlan({ go, active }: { go: (stage: Stage) => void; active: boolean }) {
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-start justify-between gap-4"><div className="space-y-1"><h3 className="text-block-title">把目标安排进可执行的任务</h3><p className="text-ui-hint text-muted-foreground">任务关联目标、材料和完成条件；支持改期、调序与跳过说明。</p></div><Button variant="outline" onClick={() => go("goals")}>查看目标</Button></div>
    {!state.tasks.length ? <LearningEmpty title="先启用一个学习目标" action={<Button onClick={() => go("goals")}>前往目标规划</Button>}>从已启用的目标创建学习计划。每项任务都保留对应目标和完成条件，避免无依据地堆叠练习。</LearningEmpty> : <>
      <div className="grid items-end gap-5 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]"><div className="max-w-sm space-y-2"><div className="flex justify-between text-ui-body"><span>任务完成</span><span className="tabular-nums">{done} / {state.tasks.length}</span></div><Progress aria-label="学习任务完成比例" value={done} max={state.tasks.length} /><p className="text-ui-hint text-muted-foreground">已跳过 {state.tasks.filter(item => item.status === "skipped").length} 项，不计入已完成。总预计 {state.tasks.reduce((sum, item) => sum + item.minutes, 0)} 分钟。</p></div><div className="flex flex-wrap items-end gap-3"><Field className="min-w-0 flex-1"><FieldLabel htmlFor="learning-task-search">搜索任务</FieldLabel><Input id="learning-task-search" type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder="任务名称或完成条件" /></Field><QuestionSelect label="任务状态筛选" value={filter} onChange={setFilter} items={[{ value: "all", label: "全部状态" }, ...statuses]} /></div></div>
    ...
    <QuestionWorkPanel id="learning-task-resource" open={active && !!resource} title={resource?.title ?? "学习材料"} description={resource?.condition ?? ""} onClose={() => setResource(null)} footer={<Button variant="outline" onClick={() => setResource(null)}>返回计划</Button>}>
      {resource?.resource === "review" && <div className="space-y-5"><QuestionContent question={compositeQuestion} /><div className="rounded-lg bg-muted/50 p-4 text-read-body"><h4 className="mb-2 text-item-title">订正依据</h4>{compositeQuestion.parts?.map(part => <section className="mb-4 space-y-2" key={part.id}><h5 className="text-item-title">第 {part.id} 问</h5>{part.explanation}</section>)}</div></div>}
      {resource?.resource === "practice" && <div className="space-y-8">{followupAttempts.map(item => <section key={item.id} className="space-y-4"><h4 className="text-item-title">{item.title} · {item.id}</h4><p className="text-read-body">{item.prompt}</p><p className="text-ui-hint text-muted-foreground">可在纸面独立作答，保留过程后由教师核验。</p><Collapsible><CollapsibleTrigger render={<Button variant="outline" size="sm" />}>查看参考作答</CollapsibleTrigger><CollapsiblePanel><p className="mt-3 border-l-2 pl-4 text-read-body">{item.answer}</p></CollapsiblePanel></Collapsible></section>)}</div>}
      {resource?.resource === "verification" && <div className="space-y-5"><p className="text-ui-hint">分别对照两份新作答与目标标准，记录具体依据。若仅完成练习但未复核，目标仍处于待验证。</p><Button onClick={() => { setResource(null); go("goals") }}>前往目标核验</Button></div>}
    </QuestionWorkPanel>
  </div>
}
