...
export type QuestionRecord = {
  id: string
  title: string
  kind: "单选题" | "多选题" | "填空题" | "解答题"
  points: number
  stem: ReactNode
  options?: { id: string; content: ReactNode }[]
  optionColumns?: 1 | 2 | 4
  figure?: ReactNode
  parts?: { id: string; content: ReactNode }[]
  answer: ReactNode
...
export function QuestionContent({ question }: { question: QuestionRecord }) {
  return <div className="prism-question-copy min-w-0 text-base leading-[1.9] text-foreground">
    ...
    {question.parts && <ol aria-label="题目小问" className="mt-5 space-y-4">{question.parts.map(part => <li key={part.id} className="flex min-w-0 gap-2"><span className="shrink-0">（{part.id}）</span><div className="min-w-0">{part.content}</div></li>)}</ol>}
  </div>
}
