...
import { LearningDiagnosis } from "./learning-diagnosis"
  ...
  return <DemoSection title="学习支持工作流" description="同一位示例学生、同一组证据：评价 → 诊断 → 目标 → 计划。四个入口共享当前页面会话，刷新后重置。">
    <div className="space-y-6">
      ...
      <Tabs key={epoch} value={stage} onValueChange={value => setStage(value as Stage)} className="gap-6"><div className="max-w-full overflow-x-auto pb-1"><TabsList variant="underline" aria-label="学习支持流程">{Object.entries(stageLabels).map(([id, label], index) => <TabsTab key={id} value={id}>{index + 1}. {label}</TabsTab>)}</TabsList></div>
        <p className="min-h-6 text-sm leading-6 text-muted-foreground" role="status" aria-live="polite">{state.message}</p>
        <TabsPanel keepMounted value="evaluation"><div className="space-y-6"><QuestionReview editor={editor} onEditorChange={setEditor} onConfirm={(scores, reason) => dispatch({ type: "review", scores, reason })} /><div className="flex flex-wrap justify-between gap-3 border-t pt-5"><p className="text-sm text-muted-foreground">确认后生成可追溯的评价版本，诊断引用具体评分点。</p><Button disabled={!review} onClick={() => setStage("diagnosis")}>继续诊断</Button></div>{!!state.reviews.length && <Collapsible><CollapsibleTrigger render={<Button variant="ghost" size="sm" />}>查看评价版本（{state.reviews.length}）</CollapsibleTrigger><CollapsiblePanel><ol className="mt-3 space-y-3 text-sm leading-6">{state.reviews.map(item => <li key={item.version}><strong>评价 v{item.version} · {Object.values(item.scores).reduce((sum, value) => sum + value, 0)} / 16</strong><p className="text-muted-foreground">{item.reason}</p></li>)}</ol></CollapsiblePanel></Collapsible>}</div></TabsPanel>
