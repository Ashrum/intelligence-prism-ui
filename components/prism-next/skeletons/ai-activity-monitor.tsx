...
export function AIActivityMonitor({ data, open, onOpenChange, triggerRef, finalFocus }: {
 ...
 return <Popover open={open} onOpenChange={onOpenChange}>
  ...
  <PopoverPopup className="ai-monitor-popup" align="end" data-shell-overlay="monitor" finalFocus={finalFocus}>
   ...
   <p className="mt-2 text-xs text-muted-foreground">{data.sourceLabel}</p>
   {data.freshness === 'stale' && data.availability === 'ready' && <p role="status" className="mt-3 text-sm text-warning-foreground leading-relaxed">状态暂未更新。以下为最后记录，任务是否继续执行尚未确认。</p>}
   {data.availability === 'unavailable' ? <Empty className="py-8"><EmptyHeader><EmptyTitle>任务服务未接入</EmptyTitle><EmptyDescription>当前无法获取你的批阅与解析任务。</EmptyDescription></EmptyHeader></Empty> : selected ? <section className="mt-4 space-y-4" aria-label="任务运行详情">
    ...
    <div className="space-y-2"><div className="flex flex-wrap items-center gap-2"><Badge variant="outline">{taskKindLabels[selected.kind]}</Badge><Badge variant={tones[selected.state]}>{taskLabels[selected.state]}</Badge></div><h3 ref={detailHeading} tabIndex={-1} className="text-base font-semibold leading-7 break-words outline-none">{selected.title}</h3><p className="text-sm text-muted-foreground">{selected.context}</p></div>
    <div className="space-y-2"><p className="text-sm font-medium">{selected.stage}{taskCountText(selected) && <span className="ml-3 tabular-nums font-normal text-muted-foreground">{taskCountText(selected)}</span>}</p><p className="text-sm leading-6 text-muted-foreground">{selected.detail}</p>{selected.followUp && <p className="text-sm text-warning-foreground">{selected.followUp}</p>}{selected.result && <p className="text-sm text-success-foreground">{selected.result}</p>}</div>
    <ol className="space-y-3" aria-label="任务执行阶段">{selected.steps.map((step,index) => <li key={`${index}-${step.label}`} className="flex items-start gap-3 text-sm leading-6"><span className="mt-1 shrink-0" aria-hidden="true">{step.state === 'done' ? <Check className="size-4 text-success-foreground"/> : step.state === 'failed' ? <CircleAlert className="size-4 text-destructive-foreground"/> : step.state === 'current' ? <Activity className="size-4 text-info-foreground"/> : <Clock3 className="size-4 text-muted-foreground"/>}</span><div className="min-w-0 flex-1"><span>{step.label}</span><span className="ml-2 text-xs text-muted-foreground">{step.state === 'done' ? '已完成' : step.state === 'current' ? '当前阶段' : step.state === 'failed' ? '失败' : '待开始'}</span></div>{step.time && <span className="shrink-0 text-xs text-muted-foreground tabular-nums">{step.time}</span>}</li>)}</ol>
    <p className="text-xs text-muted-foreground">最近更新 · {selected.updatedAt}</p>
    {data.onOpenTask ? <Button variant="outline" size="sm" onClick={() => { onOpenChange(false); data.onOpenTask?.(selected.id) }}>打开{taskKindLabels[selected.kind]}任务<ArrowUpRight/></Button> : <p className="text-xs text-muted-foreground leading-5">演示任务暂未关联业务详情。AI 处理完成不代表已复核或已发布。</p>}
   </section> : <>
    {!!data.tasks.length && <><p className="my-4 text-sm tabular-nums">{counts.running} 项运行<span className="mx-2 text-muted-foreground">·</span>{counts.queued} 项排队<span className="mx-2 text-muted-foreground">·</span>{counts.attention} 项待处理</p><div role="group" aria-label="活动筛选" className="flex flex-wrap gap-1">{([{value:'all',label:'全部'}, {value:'processing',label:'处理中'}, {value:'attention',label:'需处理'}, {value:'completed',label:'已完成'}] as const).map(item => <Button key={item.value} size="sm" variant={filter === item.value ? 'secondary':'ghost'} aria-pressed={filter === item.value} onClick={() => setFilter(item.value)}>{item.label}</Button>)}</div></>}
    {sorted.length ? <ul className="ai-monitor-list mt-3 space-y-1" aria-label="我的 AI 活动">{sorted.map(task => <li key={task.id}><Button ref={el => { if (el) rows.current.set(task.id,el); else rows.current.delete(task.id) }} variant="ghost" className="h-auto sm:h-auto w-full min-w-0 justify-start items-start gap-3 py-3 text-left whitespace-normal" aria-label={`查看${taskKindLabels[task.kind]}任务：${task.title}`} onClick={event => { listScroll.current = event.currentTarget.closest<HTMLElement>('[data-slot="popover-viewport"]')?.scrollTop ?? 0; setSelectedId(task.id) }}>
      <span className="mt-1 shrink-0 text-muted-foreground" aria-hidden="true">{task.kind === 'parsing' ? <FileSearch/> : <Activity/>}</span><span className="min-w-0 flex-1 space-y-1"><span className="flex flex-wrap items-center gap-x-2 gap-y-1"><span className="text-xs text-muted-foreground">{taskKindLabels[task.kind]}</span><Badge size="sm" variant={tones[task.state]}>{taskLabels[task.state]}</Badge></span><span className="block break-words font-medium leading-6">{task.title}</span><span className="block text-xs font-normal text-muted-foreground">{task.context}</span><span className="block text-sm font-normal text-muted-foreground leading-6">{task.followUp || task.stage}{taskCountText(task) && <span className="ml-2 tabular-nums">{taskCountText(task)}</span>}</span><span className="block text-xs font-normal text-muted-foreground">更新于 {task.updatedAt}</span></span><ChevronRight className="mt-1 shrink-0 text-muted-foreground"/>
