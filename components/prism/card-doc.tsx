...
export function CardDoc() {
  ...
  return <article className="component-article">
    <header className="component-hero">
      ...
      <p>围绕一个对象组织信息，让结果、状态与下一步操作清楚可见。</p>
    </header>

    <section className="doc-section" aria-labelledby="card-common-title">
      <div className="common-card-toolbar"><h2 id="card-common-title">常见样式</h2><div><span>界面密度</span><SegmentedControl label="Card 界面密度" size="sm" value={density} onValueChange={value => setDensity(value as CardDensity)} items={[["comfortable", "舒适"], ["compact", "紧凑"]]} /></div></div>
      <CommonCardExamples density={density} />
      <p className="doc-example-note">示例数据，修改仅在本页保留。</p>
    </section>
