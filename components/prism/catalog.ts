...
export const componentDocuments = {
  ...
  card: {
    ...
    guidance: "默认无阴影，不因 AI 来源给整卡上色。简单 Card 不强制包含状态、详情或操作区；链接和必要操作默认可识别。同一张卡片最多一条内部分隔线，展开详情时同样适用；优先用间距、对齐与字阶组织分组。选择、展开、编辑与复核使用明确控件，各对象的状态独立。加载、失败保留已有结果；长内容自然增高，不能为了对齐截断依据。",
