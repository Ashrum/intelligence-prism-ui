...
export const componentDocuments = {
  ...
  card: {
    ...
    guidance: "普通 Card 为白色，指标 Card 使用浅中性底色，16px 圆角、无阴影，静态卡片以表面和间距分组。标题图标、资源与班级标识无底色；AI Card 使用智绯侧边与来源文字，主体保持白色。简单 Card 不强制包含状态、详情或操作区；链接和必要操作默认可识别。同一张卡片最多一条内部分隔线，展开详情时同样适用；优先用间距、对齐与字阶组织分组。选择、展开、编辑与复核使用明确控件，各对象的状态独立。加载、失败保留已有结果；长内容自然增高，不能为了对齐截断依据。",
