...
export const componentDocuments = {
  ...
  card: {
    ...
    guidance: "默认无阴影，标题图标、资源与班级标识无底色，不因 AI 来源给整卡上色。简单 Card 不强制包含状态、详情或操作区；链接和必要操作默认可识别。同一张卡片最多一条内部分隔线，展开详情时同样适用；优先用间距、对齐与字阶组织分组。选择、展开、编辑与复核使用明确控件，各对象的状态独立。加载、失败保留已有结果；长内容自然增高，不能为了对齐截断依据。",
  ...
  "input-field": {
    ...
    guidance: "Input 保留原生输入行为；Field 关联标签、控件、说明与错误；TextField 将它们组合为已确认的浮动标签表现。可编辑字段常显实线边界；只读与禁用字段不增加底色，以轻实线边界和只读／不可用文字说明模式。只读内容保持清晰，禁用文字适度减弱。何时校验、何时应用以及保留哪些结果，由使用它的场景负责。",
